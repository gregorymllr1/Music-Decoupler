...
describe("ChannelStrip", () => {
    it("fires mute/solo/volume callbacks", () => {
        ...
        render(_jsx(ChannelStrip, { stem: "vocals", label: "vocals", volume: 1, muted: false, solo: false, onMute: onMute, onSolo: onSolo, onVolume: onVolume }));
...
describe("Transport", () => {
    it("toggles play and reports seek", () => {
        ...
        render(_jsx(Transport, { playing: false, currentTime: 5, duration: 60, onPlayPause: onPlayPause, onStop: vi.fn(), onSeek: onSeek, onPlaySelection: vi.fn() }));
    ...
    it("plays the selection via its own button", () => {
        ...
        render(_jsx(Transport, { playing: false, currentTime: 0, duration: 60, onPlayPause: vi.fn(), onStop: vi.fn(), onSeek: vi.fn(), onPlaySelection: onPlaySelection }));
    ...
    it("disables play selection while stems are loading", () => {
        render(_jsx(Transport, { playing: false, currentTime: 0, duration: 0, onPlayPause: vi.fn(), onStop: vi.fn(), onSeek: vi.fn(), onPlaySelection: vi.fn() }));
        const btn = screen.getByRole("button", { name: /play selection/i });
